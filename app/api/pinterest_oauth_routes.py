...
async def connect_pinterest(request: Request):
    ...
    async with app_state.db_session_factory() as _cred_db:
        _pinterest_creds = await get_oauth_app_credentials(
            _cred_db, "pinterest", project_id=state_data.get("project_id")
        )
...
async def pinterest_callback(request: Request, code: str = Query(...), state: str = Query(...)):
    ...
    async with app_state.db_session_factory() as _cred_db:
        _pinterest_creds = await get_oauth_app_credentials(_cred_db, "pinterest", project_id=project_id)
